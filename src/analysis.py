...
def data_analysis(data_ancor, data_events):

    print(f"""
    table:      data_ancor
    date start: {data_ancor['time_of_the_event'].min()}
    date end:   {data_ancor['time_of_the_event'].max()}
    period:     {(data_ancor['time_of_the_event'].max()-data_ancor['time_of_the_event'].min()).days/365:.2f} years
    customers:  {data_ancor['UUID'].nunique():,d}
    events:     {data_ancor.shape[0]:,d}
    """)
    display(data_ancor.head())

    print(f"""
    table:      data_events
    date start: {data_events['time_of_the_event'].min()}
    date end:   {data_events['time_of_the_event'].max()}
    period:     {(data_events['time_of_the_event'].max()-data_events['time_of_the_event'].min()).days/365:.2f} years
    customers:  {data_events['UUID'].nunique():,d}
    ...
    data_ancor['ancor_event_name'].value_counts().plot.barh(ax=ax[0])
    ax[0].set_title('event_name from df_ancore')
    data_events['event_name'].value_counts().plot.barh(ax=ax[1])
    ax[1].set_title('event_name from df_event')
    plt.show()


    ancor_uuid = set(data_ancor['UUID'])
    event_uuid = set(data_events['UUID'])
    ...
    data_upload_date = data_ancor['time_of_the_event'].max()

    df = data_ancor.merge(data_events, on='UUID', how='inner', suffixes=('_ancor', '_event'))
    ...
    # Let's have a look on days before first purchase
    fig, ax = plt.subplots(figsize=(20, 5))
    days_before_first_purchase = df.groupby('UUID')['days_between_purchase_and_reg'].min()
    ...
    fig, ax = plt.subplots(figsize=(20, 5))
    days_after_first_purchase = df[df['days_after_first_purch'] > 0].groupby('UUID')['days_after_first_purch'].min()
    ...
    stat.loc['users without second purchase'] = f"{(df['UUID'].nunique() - df.loc[df['days_after_first_purch'] > 0, 'UUID'].nunique())/df['UUID'].nunique():.2%}"
    display(stat)

    print(f"""You have {(df['purchase_value'] == 0).sum()/df.shape[0]:.2%} purchases with zero value""")


    # calculate ltv by days for each uuid
    periods = np.append([1,3,5], np.arange(7, 7*30+7, 7))
    for period in periods:
        df[period] = df['purchase_value'] *(df['days_between_purchase_and_reg'] <= period)
        ...
        df.loc[df['max_days_for_ltv'] < period, period] = None
    ltv_periods = df.groupby(['UUID'])[periods].sum()
    # function sum converts None to 0, so I use proxy function which set None where is necessary
    ltv_periods[df.groupby(['UUID'])[periods].max().isna()] = None


    # Let's have a look at distribution of uuid life time
    fig, ax = plt.subplots(figsize=(20, 5))
    n, bins, patches = ax.hist(df.groupby('UUID')['max_days_for_ltv'].min(), bins=30)
    ...
    sns.heatmap(ltv_periods.corr(), annot=True,
    ...
    # Calculate R2 score, but for standardised variables
    # Why? Because this score originally doesn't take into account that two variables can be in different scale and measure how good model y = x predict actual values y
    #      When we standartise data, we check how good model y = a*x + c predict actual y, which sounds more fair
    ...
    _, ax = plt.subplots(figsize=(30, 15))
    cor = ltv_periods.corr(method=r2_score_standardised)
    ...
    # Let's classify uuid and group them in 4 categories
    # - users who buy something with zero price only
    # - users with low ltv
    # - users with average ltv
    # - users with high ltv
    # We will do this classification for each period separately, because different periods have different distributions
    payers_types = pd.DataFrame()
    for col in ltv_periods.columns:
        quatile_for_type_of_payers = np.unique(np.append(0, ltv_periods.loc[ltv_periods[col] > 0, col].quantile(np.linspace(0, 1, 3 + 1))))
        ...
        payers_types[col] = pd.cut(ltv_periods[col], bins=quatile_for_type_of_payers,
        labels=['0.users_with_zero_purchases', '1.low_payers', '2.mid_payers', '3.high_payers'], include_lowest=True)


    # # I want to classify our users with 4 categories based on the data which we see for the first 7 days.
    # # Do we have the same classification for each uuid after 70 days?

    period = 7
    period_for_ltv = 7*10

    ind = ~payers_types[period_for_ltv].isna()
    pivot_count_table = payers_types.loc[ind, [period, period_for_ltv]].astype(str).reset_index().pivot_table(index=period, columns=period_for_ltv,
    ...
    for x_coordinate, column_name in enumerate([f"{period}-day Revenue", f"{period_for_ltv}-day Revenue"]):
        fig.add_annotation(
                x=x_coordinate,
                y=1.1,
                xref="x",
    ...
    _, ax = plt.subplots(figsize=(30, 15))
    cor = payers_types.replace({'0.users_with_zero_purchases':0,
                                '1.low_payers':1,
                                '2.mid_payers':2,
    ...
    _data = df.groupby('UUID')['max_days_for_ltv'].min()
    ...
    active_user_stats = df.groupby('UUID').agg({'max_days_for_ltv': 'min', 'purchase_value': 'sum'})
    ...
    active_user_stats = df.groupby('UUID').agg({'max_days_for_ltv': 'min', 'purchase_value': 'count'})
    ...
    # Let's have a look on the second component of LTV: average purchase value
    active_user_stats = df.groupby('UUID').agg({'max_days_for_ltv': 'min', 'purchase_value': 'median'})
    lifeteime_quantiles = active_user_stats['max_days_for_ltv'].quantile(np.linspace(0, 1, 20 + 1))
    active_user_stats['lifetime_bins'] = pd.cut(active_user_stats['max_days_for_ltv'],
                                                bins=lifeteime_quantiles,
                                                labels=lifeteime_quantiles.iloc[:-1].astype(str) + ' - ' + lifeteime_quantiles.shift(-1).iloc[:-1].astype(str),
    ...
    # Let's look at correlation between frequency and average purchase value
    ...
    stat = df.groupby('UUID')[['purchase_value', 'max_days_for_ltv']].agg({'purchase_value': ['mean', 'count'], 'max_days_for_ltv': 'min'})
